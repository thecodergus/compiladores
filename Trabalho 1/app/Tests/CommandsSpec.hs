...
spec = do
  describe "Commands" $ do
    it "parses if command" $ do
      parseTest command "if (x < y) { x = x + 1; } else { y = y + 1; }" `shouldBe` Right (If (Rel (IdVar "x" :<: IdVar "y")) [Atrib "x" ((:+:) (IdVar "x") (Const (CInt 1)))] [Atrib "y" ((:+:) (IdVar "y") (Const (CInt 1)))])
    
    it "parses if command" $ do
      parseTest command "if (x < y) {} else {}" `shouldBe` Right (If (Rel (IdVar "x" :<: IdVar "y")) [] [])
    ...
    it "parses while command" $ do
      parseTest command "while (x < 10) {}" `shouldBe` Right (While (Rel (IdVar "x" :<: Const (CInt 10))) [])

    it "parses atrib command" $ do
      parseTest command "x = x + 1;" `shouldBe` Right (Atrib "x" ((:+:) (IdVar "x") (Const (CInt 1))))

    it "parses read command" $ do
      parseTest command "read x;" `shouldBe` Right (Leitura "x")

    it "parses print command" $ do
      parseTest command "print x + 1;" `shouldBe` Right (Imp ((:+:) (IdVar "x") (Const (CInt 1))))

    it "parses return command with expression" $ do
      parseTest command "return x + 1;" `shouldBe` Right (Ret (Just ((:+:) (IdVar "x") (Const (CInt 1)))))
    ...
    --   parseTest command "myFunc(x, y, z);" `shouldBe` Right (Proc "myFunc" [IdVar "x", IdVar "y", IdVar "z"])
