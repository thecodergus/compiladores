module Sintatico.FunctionsAndParameters where
{-
...
import Sintatico.Lexer (braces', identifier', parens', reserved', semi', whiteSpace', commaSep', type')
import Text.Parsec (Parsec, anyChar, char, many, sepBy, try, (<|>), choice, option, optional)
import Sintatico.Types (Funcao (..), Type (..), Var (..), Id, Bloco, ExprL ((:|:)), Expr, FuncaoBloco)
import Sintatico.VariableDeclarations ( expression )
import Sintatico.BlocksAndCommands (block)
